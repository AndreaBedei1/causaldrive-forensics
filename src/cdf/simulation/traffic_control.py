...
#: Preferred blueprints, tried in order. CARLA builds vary in what they ship, and
#: a scenario that fails to place a sign because one name is missing would fail
#: for a reason that has nothing to do with the experiment.
_BLUEPRINT_CANDIDATES: Dict[str, Sequence[str]] = {
    "stop": (
        "static.prop.trafficsign_stop",
        "static.prop.streetsign",
        "static.prop.streetsign01",
        "static.prop.streetsign04",
    ),
    "yield": (
        "static.prop.trafficsign_yield",
        "static.prop.streetsign",
        "static.prop.streetsign01",
        "static.prop.streetsign04",
    ),
...
def place_traffic_control(
    ...
    for sign in (traffic_control or {}).get("signs", []) or []:
        ...
        if not bool(sign.get("spawn_prop", True)):
            record["problem"] = (
                "the scenario declares this sign but does not ask for a prop, so "
                "it must already exist on the map at this location"
            )
            placed.append(record)
            continue
        ...
        if blueprint is None:
            record["problem"] = (
                "this CARLA build has none of the candidate blueprints for a "
                "{0} sign, so the approach carries no sign a camera could "
                "read".format(kind)
            )
            placed.append(record)
            LOGGER.warning("sign %s: %s", record["sign_id"], record["problem"])
